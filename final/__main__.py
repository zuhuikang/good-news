import pyglet
import os
pyglet.font.add_file(os.path.join(os.path.dirname(__file__), 'NeueHaasDisplay-Mediu.ttf'))
...
headlinePong = Headline(
    text=tokenizeForDisplay(headlines[headlineIndex]["title"]),
    color=primaryColor,
    renderWidth=WINDOW_WIDTH - 300,
)
...
def animateNextHeadline():
    ...
    headlinePong.reset(
        tokenizeForDisplay(headlines[headlineIndex]["title"]), primaryColor
    )
...
timeline.addAnimation(
    SequentialPartReplaceAnimation(headlinePong, getNLPShape, SHAPE_DELAY)
)
timeline.wait(ANIMATIONS_DELAY)
timeline.addAnimation(
    SequentialPartReplaceAnimation(headlinePong, getNLPLemma, LEMMA_DELAY)
)
timeline.wait(ANIMATIONS_DELAY)
timeline.addAnimation(
    SequentialPartReplaceAnimation(headlinePong, getNLPPos, POS_DELAY)
)
timeline.wait(ANIMATIONS_DELAY)
timeline.addAnimation(
    SequentialPartReplaceAnimation(headlinePong, getNLPTag, TAG_DELAY)
)
timeline.wait(ANIMATIONS_DELAY)
timeline.addAnimation(
    SequentialPartReplaceAnimation(headlinePong, getNLPIsStop, ISSTOP_DELAY)
)
timeline.wait(ANIMATIONS_DELAY)
...
timeline.addAnimation(
    SequentialPartReplaceAnimation(
        headlinePong, [getNLPShape, getNLPLemma], ISSTOP_DELAY, False
    )
)
timeline.wait(ANIMATIONS_DELAY2)
timeline.addAnimation(
    ToSpecificPartsAnimation(headlinePong, getSentimentTexts, LEAVE_WORDS_DELAY)
)
timeline.wait(ANIMATIONS_DELAY2)
timeline.addAnimation(
    ListSentimentRatingsAnimation(headlinePong, getNLPSentiment, FINAL_DELAY)
)
timeline.wait(ANIMATIONS_DELAY)
