...
def _normalizeWord(word: str) -> str:
    """
    Lowercase + strip surrounding punctuation so headline parts like
    "Good:" or "Wedding" compare equal to the lowercase, punctuation-free
    words TextBlob returns in sentiment_assessments (e.g. "good", "wedding").

    Without this, a single case or punctuation difference makes the list
    comparison in get_desired_part_indicies fail silently, and the word
    gets treated as "not picked up by sentiment" and hidden instead of
    matched.
    """
...
class ToSpecificPartsAnimation(Animation):
    ...
    def get_desired_part_indicies(self) -> set[int]:
        desiredParts = set()
        normalizedHeadlineParts = [
            _normalizeWord(part) for part in self._headlineParts
        ]
        for words in self._sentimentParts:
            normalizedWords = [_normalizeWord(word) for word in words]
            n = len(normalizedWords)
            for i in range(len(normalizedHeadlineParts) - n + 1):
                if normalizedHeadlineParts[i : i + n] == normalizedWords:
